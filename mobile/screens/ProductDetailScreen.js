...
import {
  ...
  export default function ProductDetailScreen({ route, navigation }) {
    ...
    const fetchDetail = async () => {
      ...
      try {
        ...
        if (data.categoryId?._id || data.categoryId) {
          ...
          const relatedRes = await api.get("/products", { params: { categoryId } });
    ...
    const selectedImage = imageGallery[activeImage] || "https://picsum.photos/300";
    const selectedTotal = Number(product.price || 0) * quantity;
    const stockTone =
      stock === 0
        ...
        >
          ...
          <View style={styles.card}>
            <View style={styles.metaTopRow}>
              <Text style={styles.categoryText}>{product.categoryId?.name || "Fashion essentials"}</Text>
            ...
            <Text style={styles.desc}>{product.description || "Sản phẩm demo cho môn học"}</Text>
            ...
            <View style={styles.selectionSummary}>
              ...
              <View style={styles.selectionPill}>
                ...
                <Text style={styles.selectionValue}>{selectedTotal.toLocaleString()} đ</Text>
              </View>
            </View>
            ...
            <View style={styles.highlightGrid}>
              ...
              <View style={styles.highlightCard}>
                ...
                <Text style={styles.highlightLabel}>Giữ trải nghiệm mua dễ hơn</Text>
              </View>
              <View style={styles.highlightCard}>
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.relatedList}>
                {relatedProducts.map((item) => (
                  <Pressable
                    ...
                    onPress={() => navigation.replace("ProductDetail", { productId: item._id })}
                  >
                    <Image source={{ uri: item.image || "https://picsum.photos/220" }} style={styles.relatedImage} />
                    <Text style={styles.relatedCardName} numberOfLines={2}>{item.name}</Text>
                    <Text style={styles.relatedCardPrice}>{Number(item.price || 0).toLocaleString()} đ</Text>
                  </Pressable>
                ))}
              </ScrollView>
            </View>
          ...
          <View style={styles.storyCard}>
            ...
            <Text style={styles.storyText}>
              Form an toàn, phối nhanh với jeans, chân váy hoặc layer áo khoác. Nếu bạn đang demo app,
              đây là kiểu nội dung giúp trang chi tiết bớt trống và nhìn giống sản phẩm thật hơn.
            </Text>
          </View>
        </Animated.View>
      </ScrollView>
  ...
  const styles = StyleSheet.create({
    ...
    image: { width: "100%", height: 320, borderRadius: 22, backgroundColor: "#eee" },
    imageBadgeRow: {
      position: "absolute",
      top: 14,
      left: 14,
    ...
    thumbImage: { width: 72, height: 72, borderRadius: 12, backgroundColor: "#eee" },
    card: {
      backgroundColor: "#fff",
      borderRadius: 22,
      padding: 16,
    ...
    metaTopRow: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", gap: 8 },
    categoryText: {
      color: "#9a3412",
      textTransform: "uppercase",
      letterSpacing: 0.8,
    ...
    highlightLabel: { color: "#9a3412", lineHeight: 18, fontFamily: FONTS.regular },
    ...
    qtyBtnText: { color: "#fff", fontSize: 22, lineHeight: 24, fontFamily: FONTS.bold },
    qtyValueBox: {
      minWidth: 58,
      height: 40,
      borderWidth: 1,
    ...
    relatedImage: { width: "100%", height: 160, borderRadius: 14, backgroundColor: "#eee" },
